...
import { MousePointer2, PenTool, Circle as CircleIcon, Square as SquareIcon, Brush, X, Scissors, Stamp, Wand2, Maximize, Minimize, Magnet, Spline } from 'lucide-react';
...
export function ContourEditor({ contours, width, height, onChange, referenceImage, roles = [], nodeTypes = [], onUndo, onRedo }: ContourEditorProps) {
    ...
    // Tools
    type ToolType = 'select' | 'pen' | 'brush' | 'circle' | 'square';
    ...
    const handleMouseMove = (e: ReactMouseEvent) => {
        ...
        // 1. Dragging Node (Select)
        if (draggingNode && group && activeTool === 'select') {
            const pt = svg.createSVGPoint();
            pt.x = e.clientX;
            pt.y = e.clientY;
            const globalToGroup = group.getScreenCTM()?.inverse();
    ...
    const ToolButton = ({ tool, icon: Icon, label }: { tool: ToolType, icon: any, label: string }) => (
        <button
            onClick={(e) => { e.stopPropagation(); setActiveTool(tool); setSelectedIdx(null); }}
            className={`p-2 rounded transition-colors ${activeTool === tool ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
            title={label}
        >
    ...
    // Render Gizmo
    const renderGizmo = () => {
        if (selectedIdx === null || selectedIdx >= localContours.length) return null;
        const contour = localContours[selectedIdx];
        const bbox = getBBox(contour);
        const min = bbox.min;
        const max = bbox.max;
    ...
    return (
        <div
            ref={containerRef}
            className={`bg-gray-900 border border-white/10 relative overflow-hidden transition-all duration-300
                ${isFullscreen ? 'fixed inset-0 z-[9999] rounded-none' : 'w-full h-full rounded-xl'}
        ...
        >
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2 z-10" onMouseDown={(e) => e.stopPropagation()}>
                <div className="bg-black/80 backdrop-blur rounded p-1 flex flex-col gap-1 border border-white/10 shadow-xl">
                    <ToolButton tool="select" icon={MousePointer2} label="Seleccionar (V)" />
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${snappingEnabled ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                ...
                <div className="bg-black/60 backdrop-blur text-xs px-3 py-2 rounded text-gray-300 pointer-events-none max-w-[200px]">
                    {activeTool === 'select' && (
                        <>
                            <div className="font-bold mb-1 text-blue-300">Modo Selección</div>
                            • Mover: Arrastrar<br />
                            • +/- Nodo: Doble Clic<br />
                            • Tipo Nodo: <b>Alt + Clic</b><br />
                            • Eliminar: Supr
                        </>
                    )}
                    {activeTool === 'pen' && (
            ...
            {selectedIdx !== null && activeTool === 'select' && (
                <div className="absolute top-16 right-4 flex flex-col gap-2 z-10 bg-black/80 backdrop-blur rounded p-2 border border-white/10 max-w-[200px]" onMouseDown={(e) => e.stopPropagation()}>
                    ...
                    >
                        <Spline className="w-3 h-3" /> Suavizar Todo
                    </button>
            ...
            {/* Top Right Controls (Fullscreen & Reset) */}
            <div className="absolute top-4 right-4 flex gap-2 z-20" onMouseDown={(e) => e.stopPropagation()}>
                <button
                    ...
                    className="p-2 bg-black/60 hover:bg-white/20 text-white rounded-lg border border-white/10 transition backdrop-blur"
                    ...
                    {localContours.map((contour, cIdx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Render Nodes (Only in Select Mode) */}
                                {activeTool === 'select' && selectedIdx === cIdx && contour.map((p, pIdx) => {
